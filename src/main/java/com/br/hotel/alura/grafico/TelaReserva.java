...
public class TelaReserva extends BaseScreen {
    ...
    public TelaReserva(){
        super("Hotel Alura - reservas", screenWidth, screenHeight);
        add(exitButton());
    }
        ...
        buttonProximo.addMouseListener(new MouseAdapter() {
            public void mousePressed(MouseEvent var1) {
                if (textoResultado.getText().trim().isEmpty() || textoResultado.getText().trim().equals("0")) {
                    new TelaAviso("Ocorreu um erro na criação da reserva", new TelaReserva());
                    return;
                }
                Reserva reserva = new Reserva(inputIn.getDate(), inputOut.getDate(), new BigDecimal(valorResultado), formapagamento.getSelectedItem().toString());
                new TelaRegistroHospede(reserva).start();
                dispose();
                
            }
        });
        
        panel.add(JChallegensKeys.createText("SISTEMA DE RESERVAS", screenWidth / 4 - 120, 50, roboto.deriveFont(20), new Color(12,138,199,255)));
